...
- To make changes permanent — and visible from any other browser or device — click **Save to GitHub** (see below) or, without a token set up, **Download songs.json** and replace the file in this repo with the one you downloaded (drag-and-drop upload on github.com's file page, or copy it over your local clone and `git push`).
- **Reload songs.json** throws away the browser-only copy and pulls the committed file back in. Use it after you've edited `songs.json` by hand, or to discard changes you don't want to keep.
...
- **Save to GitHub** / **Download setlists.json** / **Reload setlists.json** work exactly like their `songs.json` counterparts.
...
## Saving to GitHub with a token

By default this is a static site with no backend, so nothing can silently rewrite files in your repo — saving to GitHub is always something you choose to do. There are two ways to do that:

1. **Save to GitHub** (recommended) — a one-click commit straight from the page, using a GitHub Personal Access Token.
2. **Download songs.json / setlists.json** — the original manual fallback: download the file, then upload or `git push` it yourself. This always works and needs no setup, so it's kept as a backup in case a token ever stops working or you'd rather not store one in the browser.

To set up **Save to GitHub**:

1. On github.com, go to **Settings → Developer settings → Personal access tokens → Fine-grained tokens** and generate a new token.
2. Scope it to **only this one repository** (not "all repositories"), and under **Repository permissions** give it **Contents: Read and write**. No other permissions are needed.
3. In the app, click **⚙ GitHub Settings** (in the Song Library panel), and paste in the token, your GitHub username/org, the repo name, the branch (usually `main`), and confirm the paths to `songs.json` and `setlists.json` (leave the defaults if they sit at the top level of the repo). Click Save.
4. From then on, **☁ Save to GitHub** in either panel commits that file directly — no downloading or manual committing needed.

The token is stored only in this browser's local storage, the same trust level this app already assumes for its PIN: anyone with access to this browser's dev tools could read it. If that's not a trade-off you want to make, skip GitHub Settings entirely and keep using Download/Reload — they're not going anywhere. **Forget Token** in the GitHub Settings dialog clears the saved token and repo details from this browser at any time.
...
Any song can carry a few tags (genre, feel, "requests" — whatever's useful to you) — add them in the "Tags" field when editing a song, comma-separated. As you use more tags, a row of colored chips appears above the search box: click one to show only songs with that tag (click it again, or the "✕ clear" chip, to go back to everything). Tags live entirely in `songs.json` — saved gig lists (`setlists.json`) don't know or care about tags, so tagging or retagging a song never touches your saved setlists.

Tag colors are assigned automatically (a consistent color per tag name, so the same tag always looks the same) — there's no manual color picker at the moment.

**Bulk-tagging from a setlist:** if you've already built a setlist — say, a "Bluegrass Set" — click **🏷 Tag all in this list…** in the setlist panel and enter a tag name to apply it to every song currently in that list, in one go. This is a one-time action: it just adds the tag to each song right then, and doesn't keep the setlist and the tag linked afterward, so renaming or deleting the setlist later won't affect the tag.

Tags also have their own column in the CSV import/export template (see below) if you'd rather bulk-edit them in a spreadsheet.
...
Use **Export CSV** to download your current library as a CSV — open it in Google Sheets and it doubles as a template showing the exact column layout: `Title, Section, Chords, Lyrics, Notes, Tags, Layout, Text Size %, Chord Size %`.
...
- `Tags` is a comma-separated list, e.g. `bluegrass, uptempo`.
